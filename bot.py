...
async def help(ctx):
    embed = discord.Embed(
        ...
        f"!registerMe <cf_handle> --> To register your codeforces handles\n"
        f"!ac_registerMe <ac_handle> --> To register you atcoder handles\n"
        f"!unregisterMe --> To unregister yourself from the tournament\n"
        f"!showParticipants --> To display the participants\n"
        f"!showMatches --> Shows all the matches in the current round\n"
        f"!show --> Shows the current round number\n"
        f"!roundStatus <roundnumber> --> Shows the status of the current round\n"
        f"!matchUpdates --> Gives you the updates of the ongoing match in a channel\n"
        f"!stalk <tag> --> Shows details of a particular participant\n"
        f"!help --> This message\n"
        "\n"
        "FLOW OF EVENTS\n\n"
        "--> Register your cf handles(compulsary) followed by your ac handles(not compulsary)\n"
        "--> Contact your opponent of your current round to fix a match and inform one of the moderators\n"
        "--> Have your match for anytime between 10 to 180 mins on a cf/ac problemset\n"
        "--> Be the last one standing to become the LOCKOUT CHAMPION\n\n"
        " ALL THE BEST! \n"
        ,
...
async def managerHelp(ctx):
    embed = discord.Embed(
        title="HELP",
        description=f"COMMANDS :\n\n"
        f"**!channel <channel name>** --> To change the channel of tourney manager\n"
        f"**!startRegister <channel name> <tourneyname>** --> To register a tournament with a channel\n"
        f"**!matchChannel <channel name> <tourneyname>** --> To register a channel for match\n"
        f"**!unRegMatchChannel <channel name>** --> To unregister a channel for match\n"
        f"**!startTourney <tourneyname>** --> Start a tournament\n"
        f"**!stopTourney <tourneyname>** --> Stop a tournament\n"
        f"**!showTourneys** --> Shows the list of tournaments\n"
        "\n"
        f"**!startMatch <tag1> <tag2> <rating/toughness>** --> Start a cf match between two participants\n"
        "--> To start a cf match give rating number\n"
        "--> To start a ac match give toughness (easy, medium, hard)\n"
        ,
        color=discord.Color.green())
    await ctx.send(embed=embed)
    return
...
async def stopTourney(ctx, tourneyName= "--"):
    ...
    if thisServer['tournaments'][tourneyName]["current_round"] != "finished":
        ...
        embed = discord.Embed(
            ...
            description=f"Congratulations! <@{var}> you have won the tournament :D",
            color=discord.Color.green()
        )
...
def validate_acc(handle,string):
    ...
    while True:
        ...
        firstName = parse_json['result'][0]['firstName']
